...
double KGV::Procedural::PerlinNoise::noiseDP(double x, double y, double z) {
    ...
    double  x1 = lerp(gradient3(aaa, x, y, z),				// The gradient3 function calculates the dot product between a pseudorandom
                      gradient3(baa, x - 1, y, z),				// gradient3 vector and the vector from the input coordinate to the 8
                  u);										// surrounding points in its unit cube.
    double  x2 = lerp(gradient3(aba, x, y - 1, z),				// This is all then lerped together as a sort of weighted average based on the faded (u,v,w)
                      gradient3(bba, x - 1, y - 1, z),				// values we made earlier.
                  u);
...
double KGV::Procedural::PerlinNoise::noiseDP(double x, double y) {
    ...
    // blend the results together using linear interpolation but with the curves generated by the fad function.
    // Probably could use SIMD like in the F32 version but thats for a later time.
    double  x1 = lerp(gradient2(aa, x, y),				// The gradient3 function calculates the dot product between a pseudorandom
                      gradient2(ba, x - 1, y),				// gradient3 vector and the vector from the input coordinate to the 8
                          u);										// surrounding points in its unit cube.
    double  x2 = lerp(gradient2(ab, x, y - 1),				// This is all then lerped together as a sort of weighted average based on the faded (u,v,w)
                      gradient2(bb, x - 1, y - 1),				// values we made earlier.
                          u);
