...
{
	// Get the pointer to the external WndProc thats stored inside the extra bytes of the window.
...
WindowWin32::WindowWin32( IWndProc* procObj, const std::wstring& className, S32 width, S32 height, S32 xPos, S32 yPos,
	...
	Window( width, height, xPos, yPos, caption, showWindow )
{
	...
	wndHandle = CreateWindowEx( NULL, 
								className.c_str(),
								std::wstring( windowCaption.begin(), windowCaption.end() ).c_str(),
								WS_OVERLAPPEDWINDOW,
								xPos, yPos,
								width, height,
								NULL,
...
void WindowWin32::resizeWindow( S32 width, S32 height ) 
{
	...
	SetWindowPos( wndHandle, NULL, 0, 0, width, height, SWP_NOMOVE | SWP_NOZORDER ); 
...
void WindowWin32::repositionWindow( S32 xPos, S32 yPos ) 
{
	...
	SetWindowPos( wndHandle, NULL, xPos, yPos, 0, 0, SWP_NOSIZE | SWP_NOZORDER );
